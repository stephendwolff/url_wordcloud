...
function parseText(t) {
    // Jason Davies script parses the text, and generates the word cloud.
    tags = {};
    var e = {}, n = false;
    t.split(n ? /\n/g : wordSeparators).forEach(function (t) {
        discard.test(t) || (n || (t = t.replace(punctuation, "")), stopWords.test(t.toLowerCase()) || (t = t.substr(0, maxLength),
            e[t.toLowerCase()] = t,
...
function updateTags(newTags) {
    ...
    for (var key in newTags) {
        ...
        tags[key = key.toLowerCase()] = (tags[key] || 0) + 1
    }
    tags = d3.entries(newTags).sort(function (t, e) {
        return e.value - t.value
    }), tags.forEach(function (t) {
        t.key = e[t.key]
    }), generate()
...
d3.select("#random-palette").on("click", function () {
    ...
    function t() {
        c = 5, u = Math.max(-90, Math.min(90, -60)), i = Math.max(-90, Math.min(90, 60)), e()
    }
    ...
    t(), parseText(d3.select("#text").property("value"))
}();

//d3.select(window).on("load",  parseText(d3.select("#text").property("value")));